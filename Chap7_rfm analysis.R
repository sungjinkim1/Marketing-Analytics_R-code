...
library(rfm)
# 
# rfm_data_orders = rfm_data_orders
# date <- lubridate::as_date("2006-12-31", tz = "UTC")
# min(rfm_data_orders$order_date)
# max(rfm_data_orders$order_date)
# 
# report <- rfm_table_order(rfm_data_orders, customer_id, order_date, revenue, date)
# report
# class(report)

# rfm_data_customer = rfm_data_customer
...
rfm_order_dist(package_results)
rfm_rm_plot(package_results)
...
## Drawing quintile example figure---------- 

# library(readxl)
# quintile <- read_excel("F:/Teaching/Marketing Analytics_2024 Spring_SK/quintile.xlsx")
# quintile_fig = 
# ggplot(data = quintile , aes(x = quintile, y = response_rate)) +
#       geom_bar(stat="identity") + 
#   ggtitle("Plot of Response Rates across Quintiles") +
#   xlab("Quintile") + ylab("Response Rates") + 
#   theme(axis.title.x = element_text(size=14, face="bold"),
#           axis.title.y = element_text(size=14, face="bold"))
# ggsave(filename = "quintile_fig.png",plot = quintile_fig, width = 7, height = 6)  
